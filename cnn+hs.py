...
def Pitch_Adjusting(new_harmony):
    par_idxselect = random.randint(0, 9)
    if par_idxselect == 0:
        new_harmony[0] = random.choice(size_filter)
...
def CNN(HMlist):
    base_dir = 'E:/Experiment/2021.04.Face_Recognition/0.data'
    train_dir = base_dir + '/' + 'EX_ALL/train'
    test_dir = base_dir + '/' + 'EX_ALL/test'
...
for iteration in range(itr):
    ...
    new_harmony.append(0)
    HM.append(new_harmony) # 추가

    for i in range(HMS+1): # 피트니스 계산
        HM[i][-1] = CNN(HM[i])
